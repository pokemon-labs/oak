...
def test_consistency():
    ...
    assert torch.all(
        cpp_params["pokemon_net.fc0"][0] == torch_network.pokemon_net.fc0.layer.weight
    )
    assert torch.all(
        cpp_params["pokemon_net.fc0"][1] == torch_network.pokemon_net.fc0.layer.bias
    )
    assert torch.all(
        cpp_params["pokemon_net.fc1"][0] == torch_network.pokemon_net.fc1.layer.weight
    )
    assert torch.all(
        cpp_params["pokemon_net.fc1"][1] == torch_network.pokemon_net.fc1.layer.bias
    )
    assert torch.all(
        cpp_params["active_net.fc0"][0] == torch_network.active_net.fc0.layer.weight
    )
    assert torch.all(
        cpp_params["active_net.fc0"][1] == torch_network.active_net.fc0.layer.bias
    )
    assert torch.all(
        cpp_params["active_net.fc1"][0] == torch_network.active_net.fc1.layer.weight
    )
    assert torch.all(
        cpp_params["active_net.fc1"][1] == torch_network.active_net.fc1.layer.bias
    )
    assert torch.all(
        cpp_params["moves_net.fc0"][0] == torch_network.moves_net.fc0.layer.weight
    )
    assert torch.all(
        cpp_params["moves_net.fc0"][1] == torch_network.moves_net.fc0.layer.bias
    )
    assert torch.all(
        cpp_params["moves_net.fc1"][0] == torch_network.moves_net.fc1.layer.weight
    )
    assert torch.all(
        cpp_params["moves_net.fc1"][1] == torch_network.moves_net.fc1.layer.bias
    )
    assert torch.all(
        cpp_params["main_net.fc0"][0] == torch_network.main_net.fc0.layer.weight
    )
    assert torch.all(
        cpp_params["main_net.fc0"][1] == torch_network.main_net.fc0.layer.bias
    )
    assert torch.all(
        cpp_params["main_net.fc1"][0] == torch_network.main_net.fc1.layer.weight
    )
    assert torch.all(
        cpp_params["main_net.fc1"][1] == torch_network.main_net.fc1.layer.bias
    )
    assert torch.all(
        cpp_params["main_net.value_fc2"][0]
        == torch_network.main_net.value_fc1.layer.weight
    )
    assert torch.all(
        cpp_params["main_net.value_fc2"][1]
        == torch_network.main_net.value_fc1.layer.bias
    )
    assert torch.all(
        cpp_params["main_net.value_fc3"][0]
        == torch_network.main_net.value_fc2.layer.weight
    )
    assert torch.all(
        cpp_params["main_net.value_fc3"][1]
        == torch_network.main_net.value_fc2.layer.bias
    )
    assert torch.all(
        cpp_params["main_net.p1_policy_fc2"][0]
        == torch_network.main_net.policy1_fc1.layer.weight
    )
    assert torch.all(
        cpp_params["main_net.p1_policy_fc2"][1]
        == torch_network.main_net.policy1_fc1.layer.bias
    )
    assert torch.all(
        cpp_params["main_net.p1_policy_fc3"][0]
        == torch_network.main_net.policy1_fc2.layer.weight
    )
    assert torch.all(
        cpp_params["main_net.p1_policy_fc3"][1]
        == torch_network.main_net.policy1_fc2.layer.bias
    )
    assert torch.all(
        cpp_params["main_net.p2_policy_fc2"][0]
        == torch_network.main_net.policy2_fc1.layer.weight
    )
    assert torch.all(
        cpp_params["main_net.p2_policy_fc2"][1]
        == torch_network.main_net.policy2_fc1.layer.bias
    )
    assert torch.all(
        cpp_params["main_net.p2_policy_fc3"][0]
        == torch_network.main_net.policy2_fc2.layer.weight
    )
    assert torch.all(
        cpp_params["main_net.p2_policy_fc3"][1]
    ...
    for buffer, n_frames in buffer_list[:max_games]:
        ...
        cpp_output = oak.torch.OutputBuffer(
            oak.search.cpp_inference(
                battle_frames,
                network,
                p1_cache,
                p2_cache,
